...
describe("Realmline original two-player rules", () => {
  ...
  it("rejects out-of-bounds placement without mutating the state", () => {
    const state = createInitialState();
    const snapshot = globalThis.structuredClone(state);

    assert.throws(() => placePiece(state, { row: -1, col: 0 }), /empty board cell/);
    assert.throws(() => placePiece(state, { row: 7, col: 0 }), /empty board cell/);
  ...
  it("does not allow the non-current player to move", () => {
    const state = completePlacement(createInitialState());

    assert.deepEqual(getLegalMoves(state, "B1"), []);
    assert.throws(() => movePiece(state, "B1", { row: 1, col: 5 }), /cannot move/);
  });
  ...
  it("places a permanent wall on both sides of a shared edge and advances the turn", () => {
    const moved = movePiece(completePlacement(createInitialState()), "R1", { row: 0, col: 1 });
    const snapshot = globalThis.structuredClone(moved);
    const afterWall = placeWall(moved, { row: 0, col: 1 }, "right");

    assert.deepEqual(moved, snapshot, "placing a wall does not mutate the previous state");
  ...
  it("returns a browser-ready summary without exposing mutable state internals", () => {
    const state = completePlacement(createInitialState());
    ...
    assert.deepEqual(summary, {
      ...
      score: { Red: 0, Blue: 0 },
      unclaimedTerritory: 49,
      winner: undefined
    });
    assert.equal("pieces" in summary, false);
  ...
  it("treats a fully trapped current player as game over", () => {
