...
const turnCard = turnLabel.closest("div");
...
function restartGame() {
  ...
  render();
  if (gameStarted) focusFirstPlacementTarget();
}

function render() {
  const focusedCell = document.activeElement?.closest?.(".cell");
  const focusedPosition = focusedCell && boardEl.contains(focusedCell)
    ? { row: Number(focusedCell.dataset.row), col: Number(focusedCell.dataset.col) }
    : undefined;
  ...
  renderWallControls();
  renderUndoButton();
  if (focusedPosition) focusCell(focusedPosition.row, focusedPosition.col);
  lastAnimatedPieceId = undefined;
  lastAnimatedWallKey = undefined;
}

function focusCell(row, col) {
  boardEl.querySelector(`.cell[data-row="${row}"][data-col="${col}"]`)?.focus();
}

function focusFirstPlacementTarget() {
  for (let row = 0; row < state.size; row += 1) {
    for (let col = 0; col < state.size; col += 1) {
      if (!getPieceAt(state, { row, col })) {
        focusCell(row, col);
        return;
      }
    }
...
function focusFirstCurrentPlayerPiece() {
  const piece = state.pieces.find((candidate) => candidate.player === state.currentPlayer);
  if (piece) focusCell(piece.row, piece.col);
}

function focusFirstWallControl() {
  wallControlsEl.querySelector("button:not(:disabled)")?.focus();
}
...
function getCellLabel(row, col, piece) {
  const coordinate = `${COLUMN_LABELS[col]}${state.size - row}`;
  const details = [`Cell ${coordinate}`];

  if (piece) {
    details.push(`${piece.player} piece ${piece.id}`);
  } else {
    details.push("empty");
  }

  if (state.phase === "placement" && !piece) details.push("legal placement target");
  if (piece?.id === selectedPieceId) details.push("selected piece");
  if (isMoveTarget(row, col)) details.push("legal move target");
  if (isWallSource(row, col)) details.push("moved piece, choose a wall direction");
...
function renderTurn(summary) {
  updateTurnTheme(summary);
  ...
  text.textContent = summary.phase === "finished" ? "Finished" : summary.turn;
  if (summary.turn === "Red" && summary.phase !== "finished") text.style.color = "#c32f4b";
  if (summary.turn === "Blue" && summary.phase !== "finished") text.style.color = "#6558f5";

  turnLabel.append(dot, text);
}

function updateTurnTheme(summary) {
  document.body.classList.remove("turn-red", "turn-blue");
  turnLabel.classList.remove("red-turn", "blue-turn", "finished-turn");
  turnCard.classList.remove("red-turn-card", "blue-turn-card", "finished-turn-card");
  turnLabel.style.color = "";
  turnCard.style.borderColor = "";
  turnCard.style.boxShadow = "";
  turnCard.style.background = "";
  if (summary.phase === "finished") {
    turnLabel.classList.add("finished-turn");
    turnCard.classList.add("finished-turn-card");
    return;
  }

  const turnClass = summary.turn === "Red" ? "red" : "blue";
  const turnColor = summary.turn === "Red" ? "#c32f4b" : "#6558f5";
  const turnSoft = summary.turn === "Red" ? "rgba(236, 63, 93, 0.11)" : "rgba(101, 88, 245, 0.11)";
  const turnGlow = summary.turn === "Red" ? "rgba(236, 63, 93, 0.24)" : "rgba(101, 88, 245, 0.24)";
  document.body.classList.add(`turn-${turnClass}`);
  turnLabel.classList.add(`${turnClass}-turn`);
  turnCard.classList.add(`${turnClass}-turn-card`);
  turnLabel.style.color = turnColor;
  turnCard.style.borderColor = turnColor;
  turnCard.style.background = `linear-gradient(90deg, ${turnSoft}, #ffffff 64%)`;
  turnCard.style.boxShadow = `inset 5px 0 0 ${turnColor}, 0 5px 14px ${turnGlow}`;
}
...
function handlePlacementClick(row, col) {
  ...
  render();
  if (state.phase === "placement") focusFirstPlacementTarget();
  else focusFirstCurrentPlayerPiece();
}
...
function handleWallClick(row, col, side) {
  ...
  render();
  if (state.phase === "finished") playAgainButton.focus();
  else focusFirstCurrentPlayerPiece();
}
...
globalThis.addEventListener("pagehide", stopSound);
